...
export default function Navbar({ categories }) {
  ...
  if (isAdminPage) {
    ...
    return (
      <header className="sticky top-0 bg-neutral-900 text-white border-b border-neutral-800 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <Link to="/admin" className="flex items-center gap-2 flex-shrink-0 cursor-pointer">
              ...
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-x-1.5 leading-none">
                <span className="font-extrabold text-lg text-white tracking-tight">
                  Au<span className="text-emerald-500">Kart</span>
                </span>
                <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">
                  Daily
                </span>
                <span className="text-[9px] font-bold uppercase tracking-wider text-neutral-450 bg-neutral-850 px-2 py-0.5 rounded ml-2">Admin</span>
              </div>
            </Link>

            <div className="flex items-center gap-5 text-sm font-semibold">
              <Link to="/" className="text-neutral-400 hover:text-white transition-colors cursor-pointer">
                View Site
              </Link>
              {isLoggedIn && (
                <Link to="/admin/categories" className="text-neutral-400 hover:text-white transition-colors cursor-pointer">
                  Category Manager
                </Link>
              )}
              {isLoggedIn && (
                <button
                  ...
                  className="text-red-400 hover:text-red-300 transition-colors cursor-pointer text-sm font-semibold bg-transparent border-none"
                >
                  Logout
                </button>
              )}
  ...
  return (
    <header className="sticky top-0 bg-white/85 backdrop-blur-md border-b border-neutral-100 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
